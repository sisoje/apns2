...
class APNS {
    ...
    async _sendOne(deviceToken, body, headers) {
        let options = {
    ...
    async send({deviceTokens, payload, headers}) {
        let authHeaders = Object.assign(headers, this._authHeader)
        let body = JSON.stringify(payload)
        let promises = deviceTokens.map(token => this._sendOne(token, body, authHeaders))
